...
In the next step we will add a new row that represents our import. We will use the `Add a new row` action within dataverse. Click `New Step` and enter `dataverse` in the search field. Pick the action `Add a new row`. Select `IMP_CO2_CONS_RAW_HDR` as table name. As a result the table specific tables will be shown as illustrated in the screenshot below. Mandatory fields are marked with an asteriks. The generated UI is not correct reagarding `CST_IMP_CODE`. Yes as logical primary key it is mandatory. However due to the auto generated definition no value is required:
...
Let's try to understand `Apply tp each` better. `value x` denotes the result from the previous step. To understand it you have to be aware of the JSON structure that is returned by `List rows`. Below you find a sample:
```
{
...
* UpdateContext
  ...
  It updates our local variables as in standard programming languages.
